...
while true; do
    ...
    memory_usage_kb=$(ps -o rss -p $(pgrep -d',' -f "java -Xmx1024m -jar /home/minimal-chimera-spring-0.0.2.jar") | tail -n 1)

    # Convert kilobytes to megabytes (direct arithmetic expression)
    memory_usage_mb=$((memory_usage_kb / 1024))

    cpu_usage=$(ps -o pcpu -p $(pgrep -d',' -f "java -Xmx1024m -jar /home/minimal-chimera-spring-0.0.2.jar") | tail -n 1)
